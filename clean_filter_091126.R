...
butterflies_filtered <- butterflies_raw %>% # Making a filtered dataset
  ...
  filter(wind_avg < 4,
         temp_avg >= 15,
         !(weather == "cloudy" & temp_avg < 18)) %>% # Filters for standardized butterfly conditions
  dplyr::select(month, day, week, observer, transect, lat, lon, elevation_m, time_beg, duration_s,
